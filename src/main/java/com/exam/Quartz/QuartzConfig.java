...
public class QuartzConfig {
	...
	// ExpirationDateCheckJob 클래스 정의 (Spring 관리 빈으로 등록)
	@Bean
	public JobDetail eventDateCheckJobDetail() {
		return JobBuilder.newJob(EventDateCheckJob.class)  // Job 클래스 참조
			.withIdentity("EventDateCheckJob", "DEFAULT")  // Job 이름과 그룹을 정확히 지정
			.storeDurably()  // Job이 Quartz에 의해 지속적으로 저장되도록 설정
			.build();
	}
	...
	// Trigger 정의
	@Bean
	public Trigger eventDateCheckJobTrigger() {
		return TriggerBuilder.newTrigger()
			.forJob(eventDateCheckJobDetail())  // JobDetail과 연결
			.withIdentity("EventDateCheckJobTrigger", "DEFAULT")  // Trigger 이름과 그룹을 지정
			.withSchedule(CronScheduleBuilder.cronSchedule("0 0 0/1 * * ?")) // 1시간마다 실행 (매시 정각)
			.build();
	}
	...
	public SchedulerFactoryBean scheduler() {
		...
		schedulerFactoryBean.setJobDetails(expirationDateCheckJobDetail(),
			...
			giveMonthCouponJobDetail(),
			eventDateCheckJobDetail());  // JobDetails 추가
		schedulerFactoryBean.setTriggers(expirationDateCheckJobTrigger(),
			couponExpirationCheckJobTrigger(),
			giveMonthCheckJobTrigger(),
			eventDateCheckJobTrigger());  // Triggers 추가
