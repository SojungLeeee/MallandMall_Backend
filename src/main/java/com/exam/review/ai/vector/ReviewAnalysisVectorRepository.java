...
public interface ReviewAnalysisVectorRepository extends JpaRepository<ReviewAnalysisVector, Integer> {
	...
	// 요약 기반 유사 분석 검색 (native query 사용) - product_name 추가
	@Query(value =
		"SELECT v.analysis_id, v.product_code, v.product_name, " +  // product_name 추가
	...
	// 긍정 포인트 기반 유사 검색 - product_name 추가
	@Query(value =
		"SELECT v.analysis_id, v.product_code, v.product_name, " +  // product_name 추가
	...
	// 부정 포인트 기반 유사 검색 - product_name 추가
	@Query(value =
		"SELECT v.analysis_id, v.product_code, v.product_name, " +  // product_name 추가
